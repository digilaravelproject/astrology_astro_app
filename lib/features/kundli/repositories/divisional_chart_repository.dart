...
import 'package:astro_astrologer/features/kundli/data/models/divisional_chart_model.dart';
        ...
        for (var p in pList) {
          ...
          if (pStr.isNotEmpty) {
            planets.add({
              ...
              'sign':
                  signNumber -
                  1, // Model expects 0-indexed sign, signNumber gets sign + 1
              'signName': signName,
            });
          }
        }
  ...
  Future<DivisionalChartModel?> getDivisionalChart({
    ...
    try {
      ...
      final String typeStr =
          chartType ?? (division != null ? 'd$division' : 'd1');
        ...
        final transformedJson = {
          ...
          'data': {
            'division':
                division is int
                    ? division
                    : int.tryParse(division?.toString() ?? '1') ?? 1,
            'positions': positions,
          },
        };
