  ...
  void refreshData() {
    ...
    if (user != null && user.astrologer != null) {
      ...
      selectedCategories.value =
          ast.skill?.category != null
              ? [ast.skill!.category]
              : (ast.areasOfExpertise.take(1).toList());
      selectedPrimarySkills.value =
          ast.skill?.primarySkills ?? ast.areasOfExpertise;
      selectedLanguages.value = ast.skill?.languages ?? ast.languages;
      experienceYears.value =
          (ast.skill?.experienceYears ?? ast.yearsOfExperience).toString();

      // These fields now come from the nested skill object
      selectedAllSkills.value = ast.skill?.allSkills ?? ['Prashana'];
      dailyContributionHours.value =
          (ast.skill?.dailyContributionHours ?? 10).toString();
      heardAbout.value = ast.skill?.heardAbout ?? 'Youtube';

      Logger.d(
        'AstrologerSkillsController: Data loaded from AuthController (nested skill object)',
      );
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        if (authController.currentUser.value != null) {
          ...
          Logger.d(
            'AstrologerSkillsController: Updated data saved to SharedPreferences',
          );
        }
        return true;
      } else {
            ...
            builder: (context, setModalState) {
              final filteredOptions =
                  options
                      .where(
                        (opt) =>
                            opt.toLowerCase().contains(
                              searchQuery.toLowerCase(),
                            ) ||
                            opt.tr.toLowerCase().contains(
                              searchQuery.toLowerCase(),
                            ),
                      )
                      .toList();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        onChanged:
                            (val) => setModalState(() => searchQuery = val),
